...
void MainWindow::handleAction1()
{
    ...
    QFile file(path);
    bool ret=file.open(QIODevice::ReadOnly); //只读
    if(ret==false){
        // 打开文件失败
    ...
    // 4.读取文件
    QString text=file.readAll();    // 如果是二进制文件，交给QString就不合适了
...
void MainWindow::handleAction2()
{
    ...
    QFile file(path);
    bool ret=file.open(QFile::WriteOnly);    // 写
    if(!ret){
        statusBar->showMessage(path+"打开失败！");
        return;
    }
    ...
    file.write(text.toUtf8());  // 把text转成QByteArray
